    ...
    const fetchMoreMessages = async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                setMessages((prevMessages) => [...prevMessages, ...messagesWithClassNames]);
                setOffset((prevOffset) => prevOffset + messagesWithClassNames.length);
...
const handleScroll = useCallback(() => {
    if (chatContainerRef.current) {
        ...
        console.log('isCloseToBottom:', isCloseToBottom, 'isCloseToTop:', isCloseToTop);
...
useEffect(() => {
    ...
    const handleArtistClick = async (artist) => {
        ...
        setMessages([]);
        setOffset(0);
        setHasMore(false);  // 初期メッセージを取得中はhasMoreをfalseに設定
        setSelectedArtist(artist);
    
        await fetchInitialMessages(artist.artist_id);
        markMessagesAsRead(artist.artist_id);
    
        scrollToTop(); // チャットが表示された直後にスクロール
        setHasMore(true);  // メッセージ取得が完了したらhasMoreをtrueに設定
                    ...
                    <ul>
                        {filteredArtists.map((artist) => (
                            ...
                            >
                                ...
                                {artist.name}
                                {artist.newMessages > 0 && <span className="new-message-badge">{artist.newMessages}</span>}
                            </li>
                        ))}
                    </ul>
                </div>
